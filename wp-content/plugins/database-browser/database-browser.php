...
if ( ! class_exists( 'DatabaseBrowser' ) ) {
	...
	class DatabaseBrowser {
		...
		/**
		 * Outputs the current data as a JSON document
		 * 
		 * @since 1.0
		 */
		function toJson() {
	...
	{';
		...
		/**
		 * Outputs the current data as a CSV document
		 * 
		 * @since 1.0
		 */
		function toCSV() {
		...
		/**
		 * Outputs the current data as SQL INSERT statements
		 * 
		 * @since 1.2
		 */
		function toSQL() {
		 ...
		 * @since 1.0
		 */
		function toHTML( $class = "", $truncateAtCharacter = -1 ) {
			$output = '
<table class="' . $class . '">
	<thead>
		<tr>';
